...
function AddKnowledgeModal({
  ...
  const handleSave = async () => {
    if (!form.avatarId || (!form.title && !form.sourceUrl) || (!form.content && !form.sourceUrl)) return
    setLoading(true)
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-xl">
        ...
        <p className="mt-3 text-[10px] text-muted-foreground">URLのみを指定すると、バックグラウンドで本文を取得します（社内/ローカルアドレスへのアクセスはブロックされます）。</p>
