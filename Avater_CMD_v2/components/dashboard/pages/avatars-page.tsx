...
const statusLabels: Record<string, { label: string; className: string }> = {
  ...
  MAINTENANCE: { label: "メンテ中", className: "bg-[#a78bfa]/20 text-[#a78bfa]" },
}
...
function EditAvatarModal({
  ...
  const { mutate, loading } = useApiMutation<typeof form>(`/api/avatars/${avatar.id}`, "PATCH")
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-semibold text-foreground">アバターを編集</h3>
...
export function AvatarsPage() {
  ...
  const summary = useMemo(() => {
    ...
    const maintenance = avatars.filter((a) => a.status === "MAINTENANCE").length
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          { label: "メンテ中", value: summary.maintenance.toString(), color: "text-muted-foreground" },
        ].map((s) => (
      ...
      <div className="space-y-3">
        {avatars.map((avatar) => {
          const role = avatar.role as AvatarRole
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground">
                      ...
                      <span>{roleLabels[role] || avatar.specialization}</span>
                    </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="mt-4 space-y-3">
                {allAvatars
                  ...
                  .slice(0, 4)
                  .map((conn) => {
                    const connRole = conn.role as AvatarRole
                    ...
                    return (
                      <div key={conn.id} className="flex items-center gap-3 rounded-lg bg-secondary p-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-[10px] text-muted-foreground">{roleLabels[connRole] || conn.specialization}</p>
                        </div>
...
function AvatarConfigTab({
  ...
  const { mutate: toggleAuto } = useApiMutation<{ status: string }>(`/api/automations/placeholder`, "PATCH")
